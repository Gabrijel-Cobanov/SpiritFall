...
func Enter(ctx: Player_State_Machine):
	...
	if ctx.is_grounded and ctx.direction_v == 0:
		ctx.animation_player.play("Spirit/attack_ground_swing_side_1")
	elif ctx.is_grounded and ctx.direction_v > 0:
		ctx.animation_player.play("Spirit/attack_ground_swing_up_1")
	elif !ctx.is_grounded and ctx.direction_v == 0:
		ctx.animation_player.play("Spirit/attack_air_swing_side_1")
	elif !ctx.is_grounded and ctx.direction_v > 0:
		ctx.animation_player.play("Spirit/attack_air_swing_up_1")
	elif !ctx.is_grounded and ctx.direction_v < 0:
		ctx.animation_player.play("Spirit/attack_air_swing_down_1")
