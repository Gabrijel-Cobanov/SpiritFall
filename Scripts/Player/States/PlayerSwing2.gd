...
func Pick_Attack_Anim(ctx: PlayerStateMachine):
	if ctx.CB2D.is_on_floor():
		if ctx.movement_input.y < 0:
			ctx.animator.play("swing2_up")
		else:
			ctx.animator.play("swing2_side")
	else:
		if ctx.movement_input.y < 0:
			ctx.animator.play("swing2_up_air")
		elif ctx.movement_input.y > 0:
			ctx.animator.play("swing2_down_air")
		else:
			ctx.animator.play("swing2_side_air")
